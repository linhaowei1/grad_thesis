...
def generate_combinations_and_orders(available_axioms, l, k, trial_per_kl=10000):
    """
    Generate valid axiom combinations and orders
    :param available_axioms: list of available axioms to sample from
    :param max_l: maximum length of the orders -> l: we only want one length
    :param max_k: maximum number of unique axioms in the combinations -> k: we only want one k
    ...
    for _ in range(trial_per_kl):
        ...
        combination = random.sample(list(available_axioms.keys()), k=min(k, l))
